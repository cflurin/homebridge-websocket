...
var port, plugin_name, accessories, Characteristic, addAccessory;
...
Websocket.prototype.setValue = function(accessory) {
  ...
  //this.log.debug("Websocket.onmessage: %s %s %s", name, c, data.value);
...
Websocket.prototype.sendAck = function (name, ack) {

  if (connection && this.ws.OPEN) {
    var data = {"topic":"responce", "payload": {"ack": ack, "comment": ""}};
    
    if (ack) {
      data.payload.comment =  "The new accessory '" + name + "' is now added.";
    } else {
      data.payload.comment =  "The name '" + name + "' is already used.";
    }
